...
Ext.DomHelper = function() {
	...
	return {
		...
		doInsert : function(s, u, t, v, r) {
			...
			if (this.useDom) {
				q = o(u, null);
				(r === "firstChild" ? s : s.parentNode).insertBefore(q,
						r ? s[r] : s)
			} else {
				var p = a(u);
...
Ext.util.TaskRunner = function(e) {
	...
	var k = function() {
		...
		for ( var q = 0, m = g.length; q < m; ++q) {
			...
			if (p.interval <= l) {
				var n = p.run.apply(p.scope || p, p.args
						|| [ ++p.taskRunCount ]);
				p.taskRunTime = o;
				...
				Ext.Template,
				{
					...
					applySubTemplate : function(a, k, h, d, c) {
						...
						if (m.target && Ext.isArray(l)) {
							var b = [];
							for ( var e = 0, g = l.length; e < g; e++) {
								b[b.length] = m.compiled.call(this, l[e], h,
										e + 1, g)
							}
							return b.join("")
						}
...
Ext.dd.ScrollManager = function() {
	...
	var g = function(n, m) {
		...
		i.el = n;
		i.dir = m;
		var o = (n.ddScrollConfig && n.ddScrollConfig.frequency) ? n.ddScrollConfig.frequency
				...
				Ext.data.DataReader,
				{
					...
					readRecords : function(r) {
						...
						if (!this.ef) {
							...
							for ( var x = 0; x < h; x++) {
								A = k[x];
								var C = (A.mapping !== undefined && A.mapping !== null) ? A.mapping
										: A.name;
						...
						for ( var x = 0; x < B; x++) {
							...
							for ( var w = 0; w < h; w++) {
								A = k[w];
								var l = this.ef[w](t);
								b[A.name] = A.convert((l !== undefined) ? l
										: A.defaultValue, t)
							}
							var d = new a(b, q);
							d.json = t;
					...
					readRecords : function(c) {
						...
						for ( var m = 0; m < s.length; m++) {
							...
							for ( var l = 0, w = q.length; l < w; l++) {
								var r = q.items[l];
								var g = r.mapping !== undefined
										&& r.mapping !== null ? r.mapping : l;
								var t = d[g] !== undefined ? d[g]
										: r.defaultValue;
								t = r.convert(t, d);
								u[r.name] = t
							}
			...
			onLayout : function(l, o) {
				...
				for (g = 0; g < m; g++) {
					...
					if (p.anchor) {
						...
						e = r.right ? this.adjustWidthAnchor(r.right(s), p)
								: undefined;
						b = r.bottom ? this.adjustHeightAnchor(r.bottom(k), p)
								: undefined;
						if (e || b) {
					...
					onLayout : function(d, I) {
						...
						if (!this.rendered) {
							...
							for ( var B = 0, C = x.length; B < C; B++) {
								...
								F.collapsed = false;
								if (!F.rendered) {
									F.cls = F.cls ? F.cls + " x-border-panel"
											: "x-border-panel";
									F.render(I, B)
								}
								this[o] = o != "center" && F.split ? new Ext.layout.BorderLayout.SplitRegion(
					...
					renderItem : function(e, a, d) {
						if (e && !e.rendered && e.isFormField
							...
							var b = [
									...
									this.elementStyle || "",
									typeof e.labelSeparator == "undefined" ? this.labelSeparator
											: e.labelSeparator,
				...
				Ext.Panel,
				{
					...
					initTools : function() {
						...
						if (this.closable) {
							this.addTool( {
								id :"close",
								handler :this[this.closeAction].createDelegate(
										this, [])
							})
						}
					},
		...
		.extend(
				Ext.Component,
				{
					...
					update : function(H, B) {
						...
						var z = this.maxDate ? this.maxDate.clearTime()
		...
		.extend(
				Ext.Panel,
				{
					...
					findTargets : function(c) {
						...
						return {
							close :c
									.getTarget(".x-tab-strip-close", this.strip),
							item :b,
							el :a
						}
					},
					...
					initTab : function(d, b) {
						...
						var c = e ? this.itemTpl.insertBefore(e, g)
...
Ext.PagingToolbar = Ext.extend(Ext.Toolbar, {
	...
	getPageData : function() {
		...
		return {
			total :a,
			activePage :Math
					.ceil((this.cursor + this.pageSize) / this.pageSize),
	...
	var k = function(w) {
		...
		for ( var x in q) {
			if (typeof q[x] != "function") {
				if (w[x]) {
					q[x].show();
					q[x].setText(typeof w[x] == "string" ? w[x]
...
Ext.extend(Ext.tree.AsyncTreeNode, Ext.tree.TreeNode, {
	expand : function(b, d, g) {
		...
		if (!this.loaded) {
			...
			if (a) {
				a.load(this, this.loadComplete
						.createDelegate(this, [ b, d, g ]));
				return
			}
		}
...
Ext.tree.TreeNodeUI.prototype = {
	...
	renderElements : function(e, l, k, m) {
		...
		var d = [
				...
				(l.iconCls ? " " + l.iconCls : ""),
				'" unselectable="on" />',
				g ? ('<input class="x-tree-node-cb" type="checkbox" ' + (l.checked ? 'checked="checked" />'
						: "/>"))
						: "",
...
Ext.menu.MenuMgr = function() {
	...
	function l(p) {
		...
		if (p.parentMenu) {
			p.getEl().setZIndex(
					parseInt(p.parentMenu.getEl().getStyle("z-index"), 10) + 3);
			p.parentMenu.activeChild = p
		} else {
			if (q && q.isVisible()) {
				p.getEl().setZIndex(
						parseInt(q.getEl().getStyle("z-index"), 10) + 3)
			}
		}
	}
	function b(p) {
		...
		.extend(
				Ext.form.TriggerField,
				{
					...
					onDestroy : function() {
						...
						if (this.list) {
							if (this.innerList) {
								this.innerList.un("mouseover", this.onViewOver,
										this);
								this.innerList.un("mousemove", this.onViewMove,
										this)
							}
							this.list.destroy()
						}
...
Ext.form.Checkbox = Ext.extend(Ext.form.Field, {
	...
	setValue : function(a) {
		...
		if (this.rendered) {
			...
			this.wrap[this.checked ? "addClass" : "removeClass"]
					(this.checkedCls)
		}
		if (b != this.checked) {
				...
				Ext.util.Observable,
				{
					...
					renderHeaders : function() {
						...
						for ( var c = 0; c < e; c++) {
							...
							a.css = c == 0 ? "x-grid3-cell-first "
									: (c == l ? "x-grid3-cell-last " : "");
					...
					doRender : function(d, k, u, a, t, w) {
						...
						for (y = 0, z = k.length; y < z; y++) {
							...
							if (!o || o.group != A) {
								...
								var c = typeof this.state[q] !== "undefined" ? !this.state[q]
										: this.startCollapsed;
...
Ext.extend(Ext.grid.HeaderDropZone, Ext.dd.DropZone, {
	...
	onNodeDrop : function(b, p, i, d) {
		...
		if (g != b) {
			...
			var a = Ext.lib.Dom.getRegion(b.firstChild);
			var q = (a.right - l) <= ((a.right - a.left) / 2) ? "after"
					: "before";
		...
		.extend(
				Ext.Toolbar,
				{
					...
					clearStatus : function(c) {
						...
						var b = c.useDefaults ? this.defaultText : "", a = c.useDefaults ? (this.defaultIconCls ? this.defaultIconCls
								: "")
								: "";
						if (c.anim) {
