...
public class AdultFollowupFormController extends ParameterizableViewController {
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request,
		...
		List<Obs> stiList = new ArrayList<Obs>();
		List<Concept> whosList = new ArrayList<Concept>();
		Location encLocation = Context.getLocationService()
				.getDefaultLocation();
		...
		// getting answers from concept of type coded
		...
		try {
			...
			log
					.info("Global property dataentry.oppInfTreatRes is not set to a valid concept.");
		}
		...
		List<Obs> obsProfesionList = obsService
				.getObservationsByPersonAndConcept(patient, conceptService
						.getConcept(Constants.MAIN_ACTIVITY));
		List<Obs> obsCivilStatusList = obsService
				.getObservationsByPersonAndConcept(patient, conceptService
						.getConcept(Constants.CIVIL_STATUS));
		List<Obs> obsPartnerHIVStatusList = obsService
				.getObservationsByPersonAndConcept(patient, conceptService
		...
		// patientProgram = deService.getPatientProgramByPatient(patient);
		...
		String professionObs = "";
		if (obsProfesionList.size() > 0) {
			try {
				professionObs = obsService.getObs(
						Utils.biggestObsIdNubmer(obsProfesionList)).getValueText();
			} catch (NullPointerException npe) {
				professionObs = "No name";
			}
		}
		String civilStatusObs = "";
		try {
		if (obsCivilStatusList != null && obsCivilStatusList.size() > 0)
			civilStatusObs = obsService.getObs(
					Utils.biggestObsIdNubmer(obsCivilStatusList))
					.getValueCoded().getName().getName(); 
		} catch (NullPointerException e) {
			civilStatusObs = "";
		}
		String partnerHivStatusObs = "";
		if (obsPartnerHIVStatusList.size() > 0)
			partnerHivStatusObs = obsService.getObs(
					Utils.biggestObsIdNubmer(obsPartnerHIVStatusList))
					.getValueCoded().getName().getName();
		...
		familyPlannings = conceptService.getConcept(Constants.FAMILY_PLANNING)
				.getAnswers();
//		visitReasons = conceptService.getConcept(Constants.REASON_FOR_VISIT)
//				.getAnswers();

		/*
		 * getting parameters from the form and saving them
		 */

		// checking if the form has bee submitted
		if (request.getParameter("adultRet") != null
				&& !request.getParameter("adultRet").equals("")) {

			// a list of Observation that will be storing all the observation
			// from the form
			...
			Concept nextVisitReasonConc = conceptService.getConcept(Constants.NEXT_REASON_FOR_VISIT);
			...
			if (request.getParameter("encounterTypeId").equals("-1")) {
				encType = encService.getEncounterType(Constants.ADULT_RETURN);
			} else {
		...
		mav.addObject("professionObs", professionObs);
		mav.addObject("civilStatusObs", civilStatusObs);
		mav.addObject("partnerHivStatusObs", partnerHivStatusObs);
