/*
 * SimpleDialog 1.3.5 - jQuery Plugin
...
( function($) {
	...
	$.dialog.defaults = {
		...
		focus :true,
		opacity :50,
		overlayId :'createditdialog-overlay',
		overlayCss : {},
		containerId :'createditdialog-container',
		containerCss : {},
		dataId :'createditdialog-data',
		dataCss : {},
		minHeight :null,
		minWidth :null,
		maxHeight :null,
		...
		zIndex :1000,
		close :true,
		closeHTML :'<a class="dialogCloseImg" title="Close"></a>',
		closeClass :'createditdialog-close',
		escClose :true,
		overlayClose :false,
		position :null,
		persist :false,
	...
	$.dialog.impl = {
		...
		bindEvents : function() {
			...
			if (s.o.dialog && s.o.close && s.o.overlayClose) {
				s.d.overlay.bind('click.createditdialog', function(e) {
					e.preventDefault();
					s.close();
				});
			}
		...
		getVal : function(v) {
			return v == 'auto' ? 0 : v.indexOf('%') > 0 ? v : parseInt(v
					.replace(/px/, ''));
		},
		setContainerDimensions : function(resize) {
			...
			if (!resize || (resize && s.o.autoResize)) {
				...
				var mh = s.o.maxHeight && s.o.maxHeight < w[0] ? s.o.maxHeight
						: w[0], mw = s.o.maxWidth && s.o.maxWidth < w[1] ? s.o.maxWidth
						: w[1];
				if (!ch) {
					if (!dh) {
						ch = s.o.minHeight;
